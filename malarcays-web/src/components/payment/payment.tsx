...
function makePaymentCall(accountNumber: string, recipient: string, transactionAmount: number, reference: string, {setSuccess}:{setSuccess: (showSuccess: boolean) => void}, {setFail}:{setFail: (showFail: boolean) => void}, {setClientError}:{setClientError: (message: string) => void}){
          ...
          //alert stops the thing make more professional later
            ...
            .then(function(data){
              ...
              if (fullName == entry.payee){
                ...
                makePaymentCall(accountNumber, String(entry.acc), transactionAmount, String(entry.ref), {setSuccess}, {setFail}, {setClientError});
