...
def generate_random_data(num_entries, entity_type):
    company_categories = ['Technology', 'Agriculture', 'Groceries', 'Waste Management', 'Fuel', 'Healthcare',
                          'Fashion', 'Entertainment', 'Construction', 'Transport', 'Telecommunications',
                          'Education', 'Energy', 'Automotive', 'Hospitality']
    ...
    for i in range(num_entries):
        ...
        if entity_type == 'company':
            ...
            company_data.append((None, spending_category, carbon, waste, sustainability))
...
def populate_database(num_entries, entity_type):
    ...
    if connection:
        details_data, company_data, account_data = generate_random_data(num_entries, entity_type)

        details_query = """
        INSERT INTO details (name, last_name)
        VALUES (%s, %s)
        """
        details_ids = insert_data(connection, details_query, details_data, return_last_ids=True)
        ...
        if entity_type == 'company' and company_data:
            ...
            VALUES (%s, %s, %s, %s, %s)
            """
            company_data_with_ids = [(details_ids[i], company_data[i][1], company_data[i][2], company_data[i][3], company_data[i][4]) for i in range(num_entries)]
            ...
            company_id_map = {details_ids[i]: company_ids[i][0] for i in range(num_entries)}
            account_data = [(details_ids[i], company_id_map[details_ids[i]], 2) for i in range(num_entries)]
        else:
...
if __name__ == "__main__":
    ...
    entity_type = input("Enter 'company' to generate companies or 'customer' to generate customers: ").lower()
